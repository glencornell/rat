...
static const char cvsid[] =
	"$Id: ui_send_rtp.c 4084 2007-07-27 16:16:37Z piers $";
...
static void ui_info_update_sdes(session_t *sp, const char *addr, const char *item, const char *val, uint32_t ssrc)
{
	char *arg;
...
void ui_send_rtp_cname(session_t *sp, const char *addr, uint32_t ssrc)
{
	ui_info_update_sdes(sp, addr, "rtp.source.cname", rtp_get_sdes(sp->rtp_session[0], ssrc, RTCP_SDES_CNAME), (unsigned long)ssrc);
}

void ui_send_rtp_name(session_t *sp, const char *addr, uint32_t ssrc)
{
	const char 	*sdes;
	char		 ssrc_c[9];
...
void
ui_send_rtp_gain(session_t *sp, const char *addr, uint32_t ssrc)
{
        pdb_entry_t *pdbe;
	if (!sp->ui_on) return;
...
void
ui_send_rtp_mute(session_t *sp, const char *addr, uint32_t ssrc)
{
        pdb_entry_t *pdbe;
	if (!sp->ui_on) return;
        if (pdb_item_get(sp->pdb, ssrc, &pdbe)) {
...
void
ui_send_rtp_remove(session_t *sp, const char *addr, uint32_t ssrc)
{
	if (!sp->ui_on) return;
...
void
ui_send_rtp_active(session_t *sp, const char *addr, uint32_t ssrc)
{
	if (!sp->ui_on) return;
...
void
ui_send_rtp_inactive(session_t *sp, const char *addr, uint32_t ssrc)
{
	session_validate(sp);
	if (!sp->ui_on) return;
...
void
ui_send_rtp_packet_loss(session_t *sp, const char *addr, uint32_t srce, uint32_t dest, int loss)
{
	if (!sp->ui_on) return;
...
void
ui_send_rtp_rtt(session_t *sp, const char *addr, uint32_t ssrc, double rtt_sec)
{
	if (!sp->ui_on) return;
...
void
ui_send_rtp_ssrc(session_t *sp, const char *addr)
{
	if (!sp->ui_on) return;
...
void
ui_send_rtp_addr(session_t *sp, const char *addr)
{
...
void
ui_send_rtp_title(session_t *sp, const char *addr)
{
	char	*title;
...
void ui_send_rtp_app_site(session_t *sp, const char *addr, uint32_t ssrc, char *siteid)
{
        char *enc_siteid;
	if (!sp->ui_on) return;
